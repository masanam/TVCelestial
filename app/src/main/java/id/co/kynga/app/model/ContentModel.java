...
public class ContentModel implements Parcelable {
	...
	public BubblePackageCategoryModel GameCategory = new BubblePackageCategoryModel();
	public BubblePackageModel Game = new BubblePackageModel();
	*/
	public ArrayList<ContentModel> list = new ArrayList<>();

	public ContentModel() {
	}

	public ContentModel(final String result) {
		try {
			final JSONArray array = new JSONArray(result);
			for (int counter = 0; counter < array.length(); counter++) {
				final ContentModel content_model = new ContentModel(array.getJSONObject(counter));
	...
	public ContentModel(final JSONObject json) {
		try {
			ID = json.getLong("ID");
			...
			if (GlobalController.isNotNull(json.getString("Game"))){
				Game = new BubblePackageModel(json.getJSONObject("Game"));
			}
			if (GlobalController.isNotNull(json.getString("GameCategory"))){
				GameCategory = new BubblePackageCategoryModel(json.getJSONObject("GameCategory"));
			}
			*/
		} catch (JSONException ex) {
			ex.printStackTrace();
	...
	public ContentModel(Parcel in) {
		...
		Game = in.readParcelable(BubblePackageModel.class.getClassLoader());
		GameCategory = in.readParcelable(BubblePackageCategoryModel.class.getClassLoader());
		*/
		in.readTypedList(list, ContentModel.CREATOR);
	}
