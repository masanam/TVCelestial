...
public class LogoutActivityAppMStar extends CommonActivity {
	public static LogoutActivityAppMStar instance;
	...
	protected void onCreate(@Nullable Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_logout_app_mstar);
		setInitial();
	}
	...
	private void doSelect(final int index) {
		...
		AppController.openTopupSummaryActivityAppMStar(this, package_data_model);
	}
	...
	private void setRequest() {
		...
		GlobalController.showLoading(this);
		URLController.signoutAppMStar(user_model.PhoneNumber, new URLManager.URLCallback() {
			@Override
			public void didURLSucceeded(int status_code, final String response) {
				...
				instance.runOnUiThread(new Runnable() {
					...
					public void run() {
						...
						//lbl_credit.setText("Credit: Rp... "+response);

						if (response.contains("success")) {
							SessionControllerAppMStar.close();
							if (MainActivityAppMStar.instance != null) {
								AppController.checkSessionAppMStar(MainActivityAppMStar.instance, true);
							}
							finish();
						}
						else
