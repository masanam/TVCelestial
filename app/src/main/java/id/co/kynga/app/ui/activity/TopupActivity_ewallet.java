...
import id.co.kynga.app.model.EWalletModel2;
import id.co.kynga.app.model.ResponseModel;
import id.co.kynga.app.model.VAModel;


public class TopupActivity_ewallet extends Activity {
	public static TopupActivity_ewallet instance;

	private ImageButton btnBack, btnPanahBCA, btnPanahMandiri, btnPanahPermata, btnPanahMandiriEcash;
	private EditText txt_amount, textFullname, textCC, textMM, textYY, textCVV ;
	private ImageButton btnBCASubmit, btnMandiriSubmit, btnPermataSubmit, btnMandiriEcashSubmit;
	private LinearLayout layoutPermataText, layoutPermataButton;
	private LinearLayout layoutBCAText, layoutBCAButton;
	private LinearLayout layoutMandiriText, layoutMandiriButton, layoutMandiriEcashText, layoutMandiriEcashButton;
	private boolean klikBank=false;
	private String clickOrigin;
	private TextView textRespon, textBalance;
	private VAModel va_model;
	private ImageButton btnBCACheckVA, btnMandiriCheckVA, btnPermataCheckVA, btnMandiriEcashCheckVA;
	...
	protected void onCreate(@Nullable Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_topup_ewallet);
		setInitial();
		//Note BCA = Telkomsel Tcash
		//Note Mandiri = Indosat Dompetku
		//Note Permata = XL Tunai
	...
	private void setInitial() {
		...
		btnPanahMandiriEcash = (ImageButton) findViewById(R.id.btnPanahMandiriEcash);
		btnMandiriSubmit = (ImageButton) findViewById(R.id.btnMandiriSubmit);
		btnMandiriEcashSubmit = (ImageButton) findViewById(R.id.btnMandiriEcashSubmit);
		...
		btnPanahMandiriEcash.setBackgroundResource(R.drawable.icon_panah_bawah);
		...
		layoutMandiriEcashText = (LinearLayout)findViewById(R.id.layoutMandiriEcashText);
		...
		layoutMandiriEcashText.setVisibility(View.GONE);
		...
		layoutMandiriEcashButton = (LinearLayout)findViewById(R.id.layoutMandiriEcashButton);
		...
		btnMandiriEcashCheckVA = (ImageButton) findViewById(R.id.btnMandiriEcashCheckVA);
		...
		setMargins (layoutMandiriEcashButton, 20, 0, 20, 10);
	...
	private void setEventListener() {
		...
		btnPanahMandiriEcash.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View view) {
				hideLayoutBankText();
				setPanahBawah();
				if(klikBank==false) {
					btnPanahMandiriEcash.setBackgroundResource(R.drawable.icon_panah_atas);
					layoutMandiriEcashText.setVisibility(View.VISIBLE);
					setMargin10();
					setMargins (layoutMandiriEcashButton, 20, 0, 20, 0);
				}else if(klikBank==true) {
					btnPanahMandiriEcash.setBackgroundResource(R.drawable.icon_panah_bawah);
					layoutMandiriEcashText.setVisibility(View.GONE);
					//setMargins (layoutMandiriButton, 30, 0, 30, 10);
		...
		layoutMandiriEcashButton.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View view) {
				setPanahBawah();
				hideLayoutBankText();
		...
		btnMandiriEcashSubmit.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View view) {
				clickOrigin = "fromMandiri";
				doPaymentMandiriEcash();
			}
		});

		btnMandiriEcashCheckVA.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View view) {
				clickOrigin = "fromCheckVAMandiri";
				checkVA();
...
//====================
	...
	private void setMargin10(){
		...
		setMargins (layoutMandiriEcashText, 20, 0, 20, 10);
...
*/
	...
	private void doPaymentBCA() {
		...
		GlobalController.showLoading(this);
		//GlobalController.showToast(user_model.PhoneNumber, 20000);
		URLController.paymentEWallet(phoneNumber, "telkomsel" ,new_cc_amount, new URLManager.URLCallback() {
			@Override
			public void didURLSucceeded(int status_code, final String response) {
				if (instance == null) {
					return;
				...
				instance.runOnUiThread(new Runnable() {
					...
					public void run() {
						...
						setValidationPaymentEWallet(response);
					}
				});
			}
...
*/
			...
			public void didURLFailed() {
				...
				instance.runOnUiThread(new Runnable() {
					...
					public void run() {
						GlobalController.closeLoading();
						//GlobalController.showRequestFailedWarning(instance);
						GlobalController.showMessage(instance,"Fail checking VA, or click Submit button first");
					}
				});
			}
		});
	...
	private void doPaymentMandiri() {
		...
		GlobalController.showLoading(this);
		//GlobalController.showToast(user_model.PhoneNumber, 20000);
		URLController.paymentEWallet(phoneNumber,"indosat", new_cc_amount, new URLManager.URLCallback() {
			@Override
			public void didURLSucceeded(int status_code, final String response) {
				if (instance == null) {
					return;
	...
	private void doPaymentMandiriEcash() {
		//final UserModel user_model = SessionController.getUser();
		final String  phoneNumber = SessionController.getPhoneNumberVer();
		String cc_amount = txt_amount.getText().toString();
		String new_cc_amount = cc_amount.replaceAll(",", "");
		...
		GlobalController.showLoading(this);
		//GlobalController.showToast(user_model.PhoneNumber, 20000);
		URLController.paymentEWallet(phoneNumber,"mandiri_ecash", new_cc_amount, new URLManager.URLCallback() {
			@Override
			public void didURLSucceeded(int status_code, final String response) {
				if (instance == null) {
					return;
	...
	private void doPaymentPermata() {
		...
		GlobalController.showLoading(this);
		//GlobalController.showToast(user_model.PhoneNumber, 20000);
		URLController.paymentEWallet(phoneNumber, "xl",new_cc_amount, new URLManager.URLCallback() {
			@Override
			public void didURLSucceeded(int status_code, final String response) {
				if (instance == null) {
					return;
...
*/
	...
	private void setValidationPaymentEWallet(final String response) { //convert response to model
		final ResponseModel response_model = new ResponseModel(response);
		if (response_model.Status == ResponseModel.StatusType.SucceededStatusType) {
			final EWalletModel2 ewaallet_model = new EWalletModel2(response_model.Result);
			GlobalController.showMessage(instance, ewaallet_model.transaction_status);
			//make update balance refresh...
			checkVAForBalance();
			checkBalance();
			//banner_model = new BannerModel(response_model.Result);
			//game_model = new BubblePackageModel(response_model.Result);
